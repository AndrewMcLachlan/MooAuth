...
public record Update(int Id, [FromBody] CreateApiListDataSource DataSource) : ICommand<ApiListDataSource>;

internal class UpdateHandler(IUnitOfWork unitOfWork, IDataSourceRepository repository, ISecretManager secretManager) : ICommandHandler<Update, ApiListDataSource>
{
    public async ValueTask<ApiListDataSource> Handle(Update command, CancellationToken cancellationToken)
    {
