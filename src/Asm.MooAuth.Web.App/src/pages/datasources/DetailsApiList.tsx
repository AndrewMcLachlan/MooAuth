...
import { useGetApiListDataSource } from "./hooks/useGetApiListDataSource";
import { useUpdateApiListDataSource } from "./hooks/useUpdateApiListDataSource";

export const DetailsApiList: React.FC = () => {

    const id = useIdParams();
    const { data: dataSource } = useGetApiListDataSource(id);

    const updateDataSource = useUpdateApiListDataSource(id);

    const onSubmit: SubmitHandler<CreateApiListDataSource> = async (data: CreateApiListDataSource) => {
    ...
    return (
        <Page title={dataSource.name} breadcrumbs={[{ text: dataSource.name, route: `/datasources/apilist/${dataSource.id}` }]}>
